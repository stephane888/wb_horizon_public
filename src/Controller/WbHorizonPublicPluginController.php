...
class WbHorizonPublicPluginController extends ControllerBase {
  ...
  public function submissionsList(Request $request) {
    ...
    foreach ($submissions as $id => $submission) {
      ...
      // if (empty($submission->getSourceEntity())) {
      // dd($submission->get, $submissions[4]->getData());
      // }
      ...
      $actions = [
        ...
        'duplicate' => [
          ...
          'weight' => 9,
          'url' => Url::fromRoute("entity.webform_submission.duplicate_form", [
            'webform' => $webform->id(),
            'webform_submission' => $submission->id()
          ], [
            'query' => [
      ...
      $rows[$id] = [
        ...
        'submit_to' => !empty($submission->getSourceEntity()) ? $submission->getSourceEntity()->toLink() : null,
        'actions' => [
          "data" => [
            "#type" => "operations",
            "#links" => $actions
   ...
   * all the webforms if "lesroidelareno" is not installed and only the webforms
   * availale in the current domain if not
   *
  ...
  protected function getWebForms() {
    ...
    return (\Drupal::moduleHandler()->moduleExists('lesroidelareno')) ? $formStorage->loadByProperties(
      [
    ...
    foreach ($webforms as $id => $webform) {
      $operations = [
        ...
        'delete' => [
          'title' => $this->t('Delete'),
          'weight' => 10,
          'url' => Url::fromRoute("entity.webform.delete_form", [
            'webform' => $webform->id()
          ], [
            'query' => [
              'destination' => $request->getPathInfo()
